...
for (const auto& item : collection) {
	...
	else if constexpr (std::is_convertible_v<decltype(item), Str>) {
		result += static_cast<Str>(item);
	}
	else {
		result += "??";
	}
...
inline Vec<Str> split(const Str& str, const Str& sep) {
	Vec<Str> components;
	size_t start = 0;
	size_t pos = 0;

	while ((pos = str.find(sep, start)) != Str::npos) {
...
// Concat impl (https://github.com/theypsilon/concat/blob/master/concat.hpp)
...
namespace { // type helpers and traits
	...
	struct does_overload_ostream_impl {
		template<typename T, typename B = decltype(
...
namespace { // concat_impl : stringstream to string helper, separator handlers, and parameter writer functions
	...
	template<typename CharT, typename W, typename S, typename... Args>
	inline void concat_impl_write_element(W& writer, const S& separator,
		const std::tuple<Args...>& tuple) {
		tuple_printer<0, sizeof...(Args) - 1>::template print<CharT>(writer, separator,
			tuple);
	}
	...
	void concat_impl_write_element(W& writer, const S& separator, const T& head,
		const Args &... tail) {
	...
	template<typename CharT, typename S, typename T, typename... Args,
		...
		concat_impl(const S& separator, T& writer, const Args &... seq) {
